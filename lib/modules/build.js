...
const build = (options = {}) => {
  ...
  // read sass
  const css = glob.sync('**/*.@(scss|sass)', { cwd: `${srcPath}/assets` });

  for(let i=0; i<css.length; i++) {
    let stylesheet = css[i];
    console.log(stylesheet);
    sass.render({
      file: `${srcPath}/assets/${stylesheet}`
    }, function(err, result) {
      if (err) {
        console.error("SASS build failed", err.formatted);
        return;
      }
      let outputFile = path.parse(stylesheet);
      outputFile.ext = ".css";
      let outputFileString = `${outputFile.dir}/${outputFile.name}.css`;
      console.log(outputFileString)
      fse.outputFile(`${outputPath}/${outputFileString}`, result.css);
    });
  }
