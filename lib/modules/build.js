...
const _indexSitePages = (files, { srcPath }) => {
  ...
  const pages = files.map((file) => {
    const pageData = frontMatter(fse.readFileSync(`${srcPath}/pages/${file}`, 'utf-8'));
  ...
  // index by tags
  const tags = pages.map((page) => page.tags).flat().map((tag) => {
...
const _buildSiteMap = (files, { url, outputPath }) => {
  ...
  xml.push('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">');
...
const _buildPage = (file, { srcPath, outputPath, site }) => {
  ...
  // add extra dir level if filename is not index
